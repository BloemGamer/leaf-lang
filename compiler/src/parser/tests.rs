...
{
	let result = parse_expr_from_str("a + b - c * d / e % f & g | h ^ i << j >> k").inspect_err(|e| println!("{e}"));
	assert!(result.is_ok());
}
...
{
	let result = parse_block_from_str("{ 'outer: for i in 0..10 { break 'outer; } }").inspect_err(|e| println!("{e}"));
	assert!(result.is_ok());
	let block = result.unwrap();
	match &block.stmts[0] {
...
{
	let result = parse_block_from_str("{ 'outer_loop: loop { break 'outer_loop; } }").inspect_err(|e| println!("{e}"));
	assert!(result.is_ok());
}
