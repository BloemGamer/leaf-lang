...
mod tests
{
	...
	// ========== Struct Init with Rest Pattern Tests ==========
	...
	fn test_desugar_struct_init_without_rest()
	{
		...
		match output {
			...
			_ => panic!("Expected unwrapped struct init"),
		}
	}

	#[test]
	fn test_desugar_struct_init_with_rest_no_base()
	{
		let mut desugarer = Desugarer::new();

		// Point { x: 1, .. } - has rest, no base, SHOULD wrap
		let expr = Expr::StructInit {
			path: Path::simple(vec!["Point".into()], Span::default()),
		...
		// Should be wrapped in a block with validation directive
		match output {
			Expr::Block(block) => {
				assert_eq!(block.stmts.len(), 1);

				// First statement should be a directive
				match &block.stmts[0] {
					Stmt::Directive(DirectiveNode { directive, .. }) => match directive {
						Directive::Custom { name, .. } => {
							assert_eq!(name, "validate_struct_rest");
						}
						_ => panic!("Expected Custom directive"),
					},
					_ => panic!("Expected directive statement"),
				}

				// Tail expression should be the actual struct init
				assert!(block.tail_expr.is_some());
				match block.tail_expr.unwrap().as_ref() {
					Expr::StructInit { fields, has_rest, .. } => {
						assert_eq!(fields.len(), 1);
						assert!(*has_rest);
					}
					_ => panic!("Expected struct init in tail"),
				}
			}
			_ => panic!("Expected block wrapper for rest validation"),
		}
	}

	#[test]
	fn test_desugar_struct_init_with_rest_and_base()
	{
		let mut desugarer = Desugarer::new();

		// Point { x: 1, ..other } - has rest WITH base, should NOT wrap
		let expr = Expr::StructInit {
			path: Path::simple(vec!["Point".into()], Span::default()),
			fields: vec![("x".into(), int_lit(1))],
			base: Some(Box::new(ident("other"))),
			has_rest: true,
			span: Span::default(),
		};
		...
		// Should NOT be wrapped since we have a base expression
		match output {
			Expr::StructInit {
			...
			_ => panic!("Expected unwrapped struct init with base"),
		}
	}

	#[test]
	fn test_desugar_struct_init_empty_with_rest()
	{
		let mut desugarer = Desugarer::new();

		// Point { .. } - only rest, no fields
		let expr = Expr::StructInit {
			path: Path::simple(vec!["Point".into()], Span::default()),
			fields: vec![],
			base: None,
		...
		// Should be wrapped with validation
		match output {
			Expr::Block(block) => {
				assert_eq!(block.stmts.len(), 1);
				assert!(matches!(block.stmts[0], Stmt::Directive(_)));
			}
	...
	#[test]
	fn test_desugar_struct_init_with_complex_field_exprs()
	{
		let mut desugarer = Desugarer::new();
		...
		// Should wrap and desugar the field expressions
		match output {
			Expr::Block(block) => {
				assert!(matches!(block.stmts[0], Stmt::Directive(_)));

				if let Some(Expr::StructInit { fields, .. }) = block.tail_expr.as_ref().map(|e| return e.as_ref()) {
					assert_eq!(fields.len(), 2);
					// Verify expressions are desugared
					assert!(matches!(fields[0].1, Expr::Binary { .. }));
					assert!(matches!(fields[1].1, Expr::Call { .. }));
				} else {
					panic!("Expected struct init in tail");
				}
			}
			_ => panic!("Expected block wrapper"),
		}
	...
	#[test]
	fn test_desugar_nested_struct_init_with_rest()
	{
		let mut desugarer = Desugarer::new();

		// Outer { inner: Inner { x: 1, .. }, .. }
		let expr = Expr::StructInit {
			path: Path::simple(vec!["Outer".into()], Span::default()),
			fields: vec![(
				"inner".into(),
				Expr::StructInit {
					path: Path::simple(vec!["Inner".into()], Span::default()),
					fields: vec![("x".into(), int_lit(1))],
					base: None,
					has_rest: true,
					span: Span::default(),
		...
		// Both inner and outer should be wrapped
		match output {
			Expr::Block(outer_block) => {
				assert!(matches!(outer_block.stmts[0], Stmt::Directive(_)));

				if let Some(Expr::StructInit { fields, .. }) = outer_block.tail_expr.as_ref().map(|e| return e.as_ref())
				{
					// Inner field should also be wrapped in a block
					match &fields[0].1 {
						Expr::Block(inner_block) => {
							assert!(matches!(inner_block.stmts[0], Stmt::Directive(_)));
						}
						_ => panic!("Expected inner struct to be wrapped in block"),
					}
				}
			}
			_ => panic!("Expected outer block wrapper"),
		}
	}

	#[test]
	fn test_desugar_struct_init_in_function_call()
	{
		let mut desugarer = Desugarer::new();
		...
		match output {
			Expr::Call { args, .. } => {
				// Argument should be wrapped in block
				assert!(matches!(args[0], Expr::Block(_)));
			}
			_ => panic!("Expected call expression"),
		}
	...
	#[test]
	fn test_desugar_struct_init_in_variable_decl()
	{
		let mut desugarer = Desugarer::new();

		// var p = Point { x: 1, .. };
		let var = VariableDecl {
			pattern: typed_ident_pattern("p", "Point"),
		...
		// Init should be wrapped
		match output.init {
			Some(Expr::Block(_)) => (),
			_ => panic!("Expected init to be wrapped in block"),
		}
	}

	#[test]
	fn test_desugar_struct_init_in_return()
	{
		let mut desugarer = Desugarer::new();
		...
		match output {
			...
			_ => panic!("Expected return with wrapped value"),
		}
	}

	#[test]
	fn test_desugar_multiple_struct_inits_with_rest()
	{
		let mut desugarer = Desugarer::new();

		// Tuple with two struct inits: (Point { x: 1, .. }, Point { y: 2, .. })
		let expr = Expr::Tuple {
			elements: vec![
				Expr::StructInit {
					path: Path::simple(vec!["Point".into()], Span::default()),
		...
		match output {
			Expr::Tuple { elements, .. } => {
				...
				assert!(matches!(elements[0], Expr::Block(_)));
				assert!(matches!(elements[1], Expr::Block(_)));
			}
			_ => panic!("Expected tuple"),
		}
	...
	#[test]
	fn test_desugar_struct_init_with_rest_in_array()
	{
		let mut desugarer = Desugarer::new();
	...
	#[test]
	fn test_desugar_struct_init_qualified_path()
	{
		let mut desugarer = Desugarer::new();

		// std::geometry::Point { x: 1, .. }
		let expr = Expr::StructInit {
			path: Path::simple(vec!["std".into(), "geometry".into(), "Point".into()], Span::default()),
			fields: vec![("x".into(), int_lit(1))],
			base: None,
			has_rest: true,
			span: Span::default(),
